...
For complex flows, the flow velocity can be measured by capturing the motion of numerous small tracer particles that faithfully follow the flow [@papantoniou1989analyzing; @nishino1989three]. Lagrangian particle tracking is one of the most established ways to extract the flow information, which offers access to long trajectories of tracer particles [@schanz2016shake; @tan2020introducing; @shnapp2022myptv; @barta2024proptv]. These long tracks in the Lagrangian framework make this method widely applicable for problems such as particle dispersion, mixing, and many transport-related quantities [@tan2022universality; @godbersen2021beauty; @qi2022fragmentation; @schroder20233d; @masuk2021towards; @tan20233d]. 

Although the 2D tracking algorithm has been widely available [@Ouellette], the 3D tracking of a high concentration of particles with large image overlap and thus large ambiguities, computed in an efficient way on high-performance computing clusters and also with a user-friendly graphic interface, has not been available. Accomplishing this task requires efficient parallelization, memory management, and state-of-the-art algorithm to utilize intensity and temporal information for tracking. First of all, the algorithm of iterative particle reconstruction is used to effectively segment and triangulate particles at high concentrations [@wieneke2012iterative].
The temporal information can be utilized by assuming the temporal coherence of established tracks in predicting the position in the next frame, an algorithm called Shake-The-Box method [@schanz2016shake; @tan2022universality].

The optimized code, known as `OpenLPT`, has been made available on Github [@tan2020introducing]. However, despite its robustness, the code was written in C++ without a user-friendly API or GUI, which poses challenges for the community to compile and use. In this work, we present a completely-new version of this code with user-friendly GUI and python-based API, dedicated to making OpenLPT more accessible to the scientific community.
